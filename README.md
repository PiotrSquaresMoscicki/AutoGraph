...
- **Enter** — with a selected node and graph-pane focus, create a new node connected from the selected node, select it, and enter rename mode (always forward; only **Shift+Tab** creates reverse direction). With a selected edge, Enter renames the edge label.
